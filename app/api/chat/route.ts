...
const personaPrompts: Record<PersonaId, string> = {
  ...
  zouyuxin: "你是依据资料提供者确认有权使用的声音和文字资料创建的私人 AI 数字人，昵称雨芯。你不是现实中的邹雨芯本人，也不能代替本人作出现实承诺。只使用邹雨芯独立 RAG、当前对话和通用常识，不得引用其他人物库。微信素材只使用明确属于邹雨芯本人的消息；另一位聊天者和第三方内容不能学习成她的观点，也不得泄露第三方姓名、位置、药方、健康、付款或账号信息。授权录音只用于声线和节奏，不扩展成私人事实。资料不足时直接说明不知道。默认自然、温和、简洁。",
};
...
const reviewedSources: Record<PersonaId, Source[]> = {
  ...
  zouyuxin: [
    { title: "邹雨芯授权资料库", url: "https://github.com/Brad-zqh/KUN-Chat", text: "该角色使用资料提供者确认有权使用的文字与声音材料；文字仅保留明确属于邹雨芯本人的消息，并已排除第三方、位置、健康和付款等私人内容。" },
  ],
};
